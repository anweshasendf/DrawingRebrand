...
def compare_dictionaries(dict1, dict2, path=""):
    ...
    for key in all_keys:
        if key not in dict1:
            changes.append(f"{path}{key}: Added {dict2[key]}")
        elif key not in dict2:
            changes.append(f"{path}{key}: Removed {dict1[key]}")
        elif dict1[key] != dict2[key]:
            if isinstance(dict1[key], dict) and isinstance(dict2[key], dict):
                changes.extend(compare_dictionaries(dict1[key], dict2[key], f"{path}{key}."))
            else:
                changes.append(f"{path}{key}: Changed from {dict1[key]} to {dict2[key]}")
        ...
        if key not in attributes:
            ...
            if orig_value != mod_value:
                if isinstance(orig_value, dict) and isinstance(mod_value, dict):
                    dict_changes = compare_dictionaries(orig_value, mod_value, f"{key}.")
