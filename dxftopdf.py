...
def create_batch_file(acad_exe_path, input_dxf, output_pdf, script_path):
    batch_content = f"""
@echo off
...
_Y
"""
    script_path = os.path.abspath("temp_plot_script.scr")
    with open(script_path, "w") as script_file:
        script_file.write(script_content)

    # Create and run the batch file
    batch_path = create_batch_file(acad_exe_path, input_dxf, output_pdf, script_path)
    try:
...
def process_directory(acad_exe_path, input_dir, output_dir):
    ...
    with open(log_file, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        # Process all DXF files in the input directory
...
input_directory = r"Modified5000_DXF"
output_directory = r"Modified5000_PDFandBW"
