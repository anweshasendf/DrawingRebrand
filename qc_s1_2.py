...
def main():
    original_dir = Path('Batch_8\Batch_8_DXF_In').resolve()
    modified_dir = Path('Batch_8\Batch_8_DXF_Out').resolve()
    ideal_dir = Path('IdealDXF').resolve()
    output_dir = Path('ComparisonOutput').resolve()
    original_dwg_dir = Path('Batch_8\Batch_8_DWG_In').resolve() 
    modified_dwg_dir = Path('Batch_8\Batch_8_DWG_Out').resolve()  

    log_file = os.path.join(output_dir, 'comprehensive_quality_check_run_test_batch8.csv')
